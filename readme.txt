...
│   └── server.py              # 6개 Tools
├── config/                    # 중앙 설정 관리
│   └── financial_constants_2025.py  # 한국 금융 상수 및 규칙
├── tests/
│   ├── test_jeoklip.py
│   ├── test_tooja.py
│   └── test_inchul.py
...
### 중앙 설정 관리
- **한국 금융 상수**: SWR, 가드레일, 세율 등
- **2024년 개정사항**: 연금소득 분리과세 한도 상향
- **시장 데이터**: 기대수익률, 변동성, 무위험수익률
- **법적 근거**: 소득세법, 조세특례제한법, 국민연금법
- **연도별 업데이트**: `financial_constants_YYYY.py` 파일 추가로 최신 수치 반영
...
### 위험 관리
- 안전인출률(SWR) 3.5% (2024년 기준)
- Guyton-Klinger 가드레일 (±20% 조정)
- 3버킷 전략 (시퀀스 리스크 완화)
- 변동성 타겟팅
- 한국 시장 특성 반영
...
### 금융 상수 업데이트
연도가 지나도 최신 금융 수치를 반영할 수 있습니다:

```bash
# 새로운 연도 설정 파일 추가
cp config/financial_constants_2025.py config/financial_constants_2026.py
...
### 업데이트 항목
- **세법 개정사항**: 연금소득 분리과세 한도, 세율 등
- **시장 데이터**: 무위험수익률, 기대수익률, 변동성
- **국민연금**: 최고 수령액, 소득대체율
- **SWR**: 시장 상황에 따른 안전인출률 조정
